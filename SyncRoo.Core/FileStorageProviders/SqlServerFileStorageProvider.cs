...
namespace SyncRoo.Core.FileStorageProviders
{
    public class SqlServerFileStorageProvider : IFileStorageProvider
    {
        ...
        public async Task<List<FileDto>> GetSourceFiles(string connectionString, long lastId, int batchSize, ILogger logger)
        {
            ...
            var result = (await connection.QueryAsync<FileDto>(@"DELETE TOP(@BatchSize) dbo.SourceFile
OUTPUT DELETED.FileName, DELETED.Size, DELETED.ModifiedTime",
        ...
        public async Task<List<FileDto>> GetTargetFiles(string connectionString, long lastId, int batchSize, ILogger logger)
        {
            ...
            var result = (await connection.QueryAsync<FileDto>(@"DELETE TOP(@BatchSize) dbo.TargetFile
OUTPUT DELETED.FileName, DELETED.Size, DELETED.ModifiedTime",
                new { batchSize })).ToList();
